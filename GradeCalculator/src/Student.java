...
public class Student {
	...
	{
		if(aSection.equalsIgnoreCase(SectionType.LABS))
		{
			this.labSum += aGrade;
			this.labCount++;
		}
		else if(aSection.equalsIgnoreCase(SectionType.LAB_REPORTS))
		{
			this.labRSum += aGrade;
			this.labRCount++;
		}
		else if(aSection.equalsIgnoreCase(SectionType.HOMEWORK))
		{
			this.hwSum += aGrade;
			this.hwCount++;
		}
		else if(aSection.equalsIgnoreCase(SectionType.FINAL))
		{
			this.finalExam = aGrade;
		}
		else if(aSection.equalsIgnoreCase(SectionType.EXAM01))
		{
			this.exam01 = aGrade;
		}
		else if(aSection.equalsIgnoreCase(SectionType.EXAM02))
		{
			this.exam02 = aGrade;
		}
		...
		while(filein.hasNextLine())
		{
			...
			switch(header)
			{
				case SectionType.LABS:
					currSection = SectionType.LABS;
				case SectionType.LAB_REPORTS:
					currSection = SectionType.LAB_REPORTS;
				case SectionType.HOMEWORK:
					currSection = SectionType.HOMEWORK;
				case SectionType.EXAM01:
					currSection = SectionType.EXAM01;
				case SectionType.EXAM02:
					currSection = SectionType.EXAM02;
				case SectionType.FINAL:
					currSection = SectionType.FINAL;
				default:
